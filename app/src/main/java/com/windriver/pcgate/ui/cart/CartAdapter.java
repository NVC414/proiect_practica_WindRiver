...
public class CartAdapter extends RecyclerView.Adapter<CartAdapter.CartViewHolder>
    ...
    public void setCartItems(List<com.windriver.pcgate.ui.cart.CartItem> cartItems)
    {
        androidx.recyclerview.widget.DiffUtil.DiffResult diffResult = androidx.recyclerview.widget.DiffUtil.calculateDiff(new androidx.recyclerview.widget.DiffUtil.Callback() {
            ...
            public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
                return CartAdapter.this.cartItems.get(oldItemPosition).getName().equals(cartItems.get(newItemPosition).getName());
            }
            @Override
            public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {
                return CartAdapter.this.cartItems.get(oldItemPosition).equals(cartItems.get(newItemPosition));
            }
        });
        this.cartItems = cartItems;
    ...
    public void onBindViewHolder(@NonNull CartViewHolder holder, int position)
        ...
        holder.addButton.setOnClickListener(v -> CartViewModel.getInstance().addItem(new CartItem(item.getName(), item.getPrice(), 1)));
        holder.removeButton.setOnClickListener(v -> CartViewModel.getInstance().addItem(new CartItem(item.getName(), item.getPrice(), -1)));
        }
